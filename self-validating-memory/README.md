...
#### 완전 task-free 조합 — 오라클 제거 (`experiment_hybrid_taskfree.py`)

위 조합은 *오라클* 컨텍스트였다. 오라클을 SVMP의 실제 기여물 — `ContextInferrer`(보상 스트림
기반 변화감지, 과제 ID 불요) — 로 대체하고 prequential 평가. 충돌 레짐(Permuted), n=4:

| 컨텍스트 | 최종 acc |
|----------|----------|
| 없음 (plain replay) | 0.571 |
| 오라클 (상한) | 0.990 |
| **추론 (task-free, SVMP)** | **0.940** — 오라클 이득의 **88% 회복** |

- ✅ **진짜 두-시스템 조합 성립**: *표준 replay* + *SVMP의 task-free 컨텍스트 발견*. 과제 ID를
  train·test 어디에도 안 쓰고 0.571→**0.940**으로, 오라클(0.99)에 근접. 잔여 갭은 part 4–5가
  정량화한 감지 노이즈(보상 기반 분절)다.
- 📌 이 88%는 SVMP 단독 task-free(part 4 ~39%, part 5 0.72)보다 훨씬 높다 — **replay가 더 강한
  운반체**라 노이지한 컨텍스트 태그로도 견고히 학습하기 때문. "운반은 replay, 컨텍스트는 SVMP"
  분업이 수치로 확인된다.
