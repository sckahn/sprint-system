...
| 컨텍스트 | A1 | B1 | A2* | B2* | 복귀 mean | probe |
|----------|----|----|-----|-----|-----------|-------|
| 오라클(정체 앎) | 0.753 | 0.990 | 0.750 | 0.995 | **0.872** | 0 |
| forward(새 슬롯) | 0.753 | 0.990 | **0.160** | 0.735 | 0.447 | 0 |
| **재인식**(보상 프로빙) | 0.753 | 0.959 | **0.696** | 0.945 | **0.821** | 240 |

- ✅ **재인식이 복귀 망각을 복원**: A2 0.16→0.70, 복귀 mean 0.447→0.821 — 오라클(0.872) 대비
  **갭의 88% 회복**. 과제 ID 없이 보상만으로 옛 과제의 원래 슬롯을 재선택. probe 240<270(이상)인
  건 복귀 시 새 슬롯을 *덜* 할당한다는 증거.
- ⚠️ **비용/범위**: 경계마다 짧은 프로빙(잔여 갭=프로빙 동안 잘못된 컨텍스트의 전이 손실). 이
  실험은 **재인식을 격리**하려 경계 신호를 가정(감지는 part 4 소관, 노이지) — 완전한 시스템은
  part 4 감지 + part 5 재인식을 합성한다.
...
RecognizingContextManager 보상 프로빙 재인식(Phase 6 + 후속)을 검증한다. 실 retriever
...
- **파국적 망각**(Phase 6): 원판 vault·게이트만으론 순차 과제 망각 못 막음(+0.94, 중심 주장
  ...
  **라벨 충돌형**(도메인 증분)에선 무효(최종 Δ−0.01). **컨텍스트 키**(`ctx_dim`)로 충돌도
  ...
  프로빙 재인식**으로 되돌아온 과제(B→A) 복원은 갭의 88%(`RecognizingContextManager`)
