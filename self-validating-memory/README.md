...
→ 망각 저항을 실제로 얻으려면 별도 메커니즘 필요(과제별 헤드, EWC식 가중치 보호, 혹은
decay 없는 verified-vault가 결정에 직접 투표). 현 설계의 vault/게이트만으론 불충분 — 후속 과제.
...
손잡이·망각 지표(Phase 6)를 검증한다. 실 retriever 실험은 다운로드가 필요해 테스트엔 미포함.
...
- **파국적 망각 미해결**(Phase 6): vault·게이트만으론 순차 과제 망각 못 막음(+0.94). 망각
  저항은 후속 메커니즘 필요 — 설계의 안정성-가소성 주장은 현 구현에서 미충족
