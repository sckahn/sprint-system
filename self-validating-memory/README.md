...
| **learned** | 학습으로 융합 | 0.673 ± 0.038 |

**정직한 결론**: 학습된 추정기는 naive 신뢰-only 평균을 분명히 이긴다(+0.038, 7/8).
그러나 **손튜닝된 robust 휴리스틱은 못 이긴다**(−0.023, robust 우세). 즉 외부 정답
신호만으로 학습하면 *쉬운 승리*(naive 베이스라인)는 회복하지만 *어려운 승리*(일관성
휴리스틱 능가)는 못 한다. **세 번째 시도도 "출처품질 평가가 가장 약한 부품"이라는
설계 경고를 재확인**한다 — 손튜닝된 일관성 prior가 여전히 최선이다.

`PYTHONPATH=. python examples/experiment_learned_trust.py` 로 재현.

> **Phase 4 종합**: ① 기본 삼각측량은 무정보 사전에서 chance 근처 → ② robust 합의는
> 적대적 레짐에서만 +0.04 → ③ 학습 추정기는 naive는 이기되 robust는 못 이김.
> 세 시도 모두 같은 결론으로 수렴: *출처 품질 평가는 진짜로 어렵다*.
...
- 학습 추정기는 naive 신뢰 베이스라인은 이기나 손튜닝 robust는 못 이김 — 출처품질
  평가는 여전히 미해결(설계가 예견한 약점)
