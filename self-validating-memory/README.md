...
- 🔬 **정직한 한계**: 입력 분포가 과제 간 동일하므로 과제 정체성은 *보상 contingency*로만 관측
  ...
  forward-only 감지기는 **되돌아온 컨텍스트**(B→A) 재인식 불가 — 오류패턴↔과거슬롯 매칭이 다음 후속.
...
손잡이·망각 지표·LabelVault 직접투표·컨텍스트 키 분리·ContextInferrer 체제전환(Phase 6 +
후속)을 검증한다. 실 retriever 실험은 다운로드가 필요해 테스트엔 미포함.
...
- **파국적 망각**(Phase 6): 원판 vault·게이트만으론 순차 과제 망각 못 막음(+0.94, 중심 주장
  ...
  **라벨 충돌형**(도메인 증분)에선 무효(최종 Δ−0.01). **컨텍스트 키**(`ctx_dim`)로 충돌도
  해결됨을 증명(오라클 0.58→0.87)했으나, 과제 ID 없는 **추론 컨텍스트**는 부분 해결(~39%)에 그침
