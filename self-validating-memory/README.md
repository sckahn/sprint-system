...
- ⚠️ **정직한 범위**: 합성 과제는 영역 분리 가능 + 인코더 키 안정이라 episodic recall이 쉽다.
  **키 표류**나 **영역 라벨 충돌**(같은 입력, 다른 정답)에선 이 메커니즘이 무너진다. 또 투표
  품질은 검증 신호(외부 reward/동결 심판)에 종속. 즉 **설계의 메모리 원칙은 결정에 닿기만 하면
  옛 과제를 지킨다**는 게 핵심 결론 — 원판의 soft-context 융합은 못 했다.
...
손잡이·망각 지표·LabelVault 직접투표(Phase 6 + 후속)를 검증한다. 실 retriever 실험은
다운로드가 필요해 테스트엔 미포함.
...
- **파국적 망각**(Phase 6): 원판 vault·게이트만으론 순차 과제 망각 못 막음(+0.94, 중심 주장
  ...
  검증 사실의 decay-free 직접 투표)으로 망각 ~2–5× 감소·최종 0.40→0.88 — 단 영역 분리·키 안정
  가정에 한함(키 표류·라벨 충돌에선 미검증)
